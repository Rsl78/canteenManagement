...
const RegistrationPage = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                const response = await axios.post("http://192.168.68.157:8083/adduser", values);
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-eggshell-white py-12 px-4 sm:px-6 lg:px-8">
            <div className="flex justify-center items-center relative z-10 w-full max-w-md">
                <div
                    className="card w-full backdrop-blur-xl bg-black/5 shadow-[0_8px_30px_rgb(0,0,0,0.12)] rounded-2xl p-8 border border-black/10">
                    <form onSubmit={formik.handleSubmit} className="space-y-6">
                        ...
                        <div>
                            ...
                            />
                            {formik.errors.username ? <div className={"text-red-600 text-xs font-semibold pt-1"}>{formik.errors.username}</div> : null}
                        </div>
